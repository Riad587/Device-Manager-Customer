package com.emi.customer.repository
...
class ShopOwnerRepo {
    ...
    suspend fun fetchShopOwnerProfile(shopId: Int): Map<String, String>? =
        withContext(Dispatchers.IO) {
            ...
            try {
                val endpointStr = _root_ide_package_.com.emi.customer.services.ApiEndpoints.getShopOwnerProfileUrl(shopId)
    ...
    suspend fun registerDeviceOnBackend(payload: JSONObject): String? = withContext(Dispatchers.IO) {
        var connection: HttpURLConnection? = null
        try {
            val urlTarget = URL(_root_ide_package_.com.emi.customer.services.ApiEndpoints.DEVICE_ADD_URL)
            connection = urlTarget.openConnection() as HttpURLConnection
    ...
    suspend fun fetchDeviceProfileByImei(payload: JSONObject): String? = withContext(Dispatchers.IO) {
        var connection: HttpURLConnection? = null
        try {
            val urlTarget = URL(_root_ide_package_.com.emi.customer.services.ApiEndpoints.DEVICE_INFO_BY_IMEI_URL)
            connection = urlTarget.openConnection() as HttpURLConnection
            connection.requestMethod = "POST"
            connection.setRequestProperty("Content-Type", "application/json")
            connection.setRequestProperty("Accept", "application/json")
    ...
    // ── 🛡️ NEW WORKER: DIRECT UNLOCK KEY FETCH (100% ERROR-FREE) ──
    /**
     * কোন থার্ডপার্টি লাইব্রেরি (OkHttp) ছাড়া সরাসরি HttpURLConnection ব্যবহার করে
     * ব্যাকএন্ড থেকে আনলক কি রিট্রিভ করার জন্য আপনার কোড আর্কিটেকচারের সাথে মানানসই মেথড।
     */
    suspend fun fetchUnlockKeyDirectly(payload: JSONObject): String? = withContext(Dispatchers.IO) {
        var connection: HttpURLConnection? = null
        try {
            // 🚨 NOTE: Replace DEVICE_INFO_BY_IMEI_URL with your custom endpoint property if needed
            val urlTarget = URL(_root_ide_package_.com.emi.customer.services.ApiEndpoints.DEVICE_UNLOCK_KEY)
            connection = urlTarget.openConnection() as HttpURLConnection
            connection.requestMethod = "POST"
            connection.setRequestProperty("Content-Type", "application/json")
            connection.setRequestProperty("Accept", "application/json")
            connection.connectTimeout = 5000 // Fast emergency unlock timeout
            ...
            Log.w("UNLOCK_KEY_REPO", "Direct api backup call network failure: ${e.message}")
            null
        } finally {
            connection?.disconnect()
        }
